function varargout = barvalues(h,precision,boolRotate)
...
t=[];
if nargin<3 || isempty(boolRotate), boolRotate = false; end
...
elseif isaType(h,'figure')
   ...
   for b =B'
           t = [t; {barvalues(b,precision,boolRotate)}]; % Return array of text objects
                                              % for each bar plot.
   end
    if nargout>0
        varargout{1}=t;
...
for hn = h 
    ...
    xData = double(hn.XData) +XOffset;
    yData = hn.YData +YOffset;
    if size(xData,1)==4
      xData=mean(xData);
      yData=yData(2,:);
    end     
    if boolRotate, xData = xData-0.04; rAngle=30; align='left'; else, rAngle = 0; align='center'; end
    t = [t;  text(xData,yData,...    %position
       arrayfun(@(x)num2str(x,precision),yData,'UniformOutput' ,false),...    %text to display
        'HorizontalAlignment',align,'VerticalAlignment','bottom', 'Rotation',rAngle)];
end
if nargout>0
    varargout{1}=t;
end
